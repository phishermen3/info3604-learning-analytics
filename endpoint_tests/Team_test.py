...
from App.views.Team import team_bp, create_team, join_team, teams
...
def test_create_team_missing_name(client):
    ...
    assert response.get_json()["error"] == "team_name is required"
...
def test_join_team_success(client):
    team_id = len(teams) + 1
    teams[team_id] = {"id": team_id, "name": "Beta", "course": 1, "code": "ABC123"}
